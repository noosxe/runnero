...
**Provider busy-veto on idle drains (RUN-182):** idle drain decisions are only as good as the local busy classification, which can be stale in one specific window — a supervisor restart closes in-flight job rows during startup recovery, so a runner that is **mid-job on the forge** briefly looks idle, and the first reconcile tick's scale-down (excess-idle, scale-to-zero, or max-concurrency prune) could deregister and terminate it. To make restarts safe, the provider gets the final word: every idle-drain deregistration is checked, and a `422 "currently running a job"` refusal — surfaced by the GitHub provider as `provider.ErrRunnerBusy` — **vetoes the termination**. The vetoed runner is kept alive and locally re-marked busy (with a job-history transition row, docs/21), so classification converges on the next sync and the runner finishes its job; remaining excess is drained from other, non-vetoed candidates. Only the typed busy error vetoes: any other deregistration failure keeps the legacy best-effort termination, with the ghost sweep (docs/20) reconciling the orphaned registration. Shutdown, pool-deletion, and hung-runner paths are explicitly exempt — those are destructive by contract and stay best-effort.
