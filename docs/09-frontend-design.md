...
Tabbed routes keep the active tab as a **path segment** (`/settings/users`, `/logs/removals`, `/account/security`) instead of component state — each tab is its own child route, so browser back/forward walks the tab history and deep links open the exact tab (RUN-283, following the /account pattern from docs/37). Data state rides search params (`/logs/supervisor?boot=…`, `/logs/runners?runner=…`). Pool detail joined with RUN-285: `/pools/:id/runners|config|renovate`, with bare `/pools/:id` canonicalizing to the runners tab. `resolveRouteTab` is gone — every tabbed route is plain routing.
...
- `/pools/$poolId` specifics (RUN-258, path segments since RUN-285): tabs are `runners` (default — bare `/pools/:id` redirects there) | `config` | `renovate`; the three tabs are shown for every role (viewer surfaces inside each are read-only, docs/35 §2.2).
