...
- **Encryption at Rest & Secret Derivation**: Sensitive credentials stored in the local SQLite database must be encrypted at rest using AES-256. The AES key is deterministically derived from the single required `SUPERVISOR_DB_ENCRYPTION_KEY` master key via HKDF-SHA256 under its own context label, so operators manage exactly one secret (implemented in `internal/keys`, RUN-9). Since the opaque-session redesign (RUN-230, docs/32) no signing secret is derived: session tokens are random values, not signed artifacts.
...
- **Session Tokens (opaque, DB-backed — docs/32 section 3)**: Admin session tokens are 32-byte `crypto/rand` values transported exclusively via an `HttpOnly` cookie with `SameSite=Strict`; only their SHA-256 hash is stored server-side. Two clocks bound every session: a sliding idle deadline (default 168h, sliding renewal past half the window, never writing before then) and an absolute cap (default 720h, fixed at issuance and never extended). `Max-Age` tracks the idle deadline so browser expiry tracks the server row. The `Secure` attribute is three-mode (`SUPERVISOR_SECURE_COOKIES=auto|always|never`, default `auto`: attached only for HTTPS requests, detecting direct TLS or `X-Forwarded-Proto: https` behind a configured trusted proxy); the legacy `SUPERVISOR_SECURE_COOKIE` boolean still works (`true` = `always`). Tokens never appear in `Authorization` headers or client-side storage. Session state is tracked in the database for visibility and forced revocation.
- **Hardened Login (docs/32 section 4)**: every login failure — unknown username and wrong password alike — feeds an in-memory rate limiter keyed by username + client IP (sliding 15-minute window; the 5th failure locks the key with exponential backoff 1, 2, 4... minutes capped at 15, answered `ResourceExhausted` with a retry hint; a successful login resets only that key). The client IP comes from the socket address; the first `X-Forwarded-For` entry is trusted only when `SUPERVISOR_TRUSTED_PROXY=true`. A bcrypt hash generated once at startup makes the unknown-username path burn the same comparison as the wrong-password path, so timing and error shape cannot reveal whether an account exists; the audit reason stays coarse (`invalid_credentials`).
- **Password Policy (docs/32 section 4.4)**: passwords are bcrypt-hashed at the configurable cost (4-31, default 12; stored hashes always compare at their own cost). The class-C floor at every set path is 12 characters — no composition rules, no forced rotation.
- **Audit & Maintenance (docs/32 section 5)**: auth decisions land in `audit_logs` (`auth.setup_admin`, `auth.login_success`, `auth.login_failed`, `auth.logout`, `auth.session_revoked`, `auth.rate_limited`, `auth.access_denied`) with the resolved client IP in a dedicated column; details never carry credentials, tokens, or challenge bytes (leakage-tested). An hourly supervisor job purges sessions past either clock and audit rows older than `SUPERVISOR_AUDIT_RETENTION` (default 90 days); a failed sweep logs a warning and never crashes the daemon.
- **Fail-Closed Authorization (docs/32 section 2.3)**: a static procedure-to-role matrix classifies every Connect RPC (public: setup, login, onboarding status; admin: everything else). An unclassified procedure is rejected with an internal error at request time, and a coverage test enumerates the generated protobuf descriptor so a new RPC cannot ship unclassified. Enforcement lives in one interceptor wrapping unary and streaming calls alike; handlers never re-check.
