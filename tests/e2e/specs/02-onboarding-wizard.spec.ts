...
test.describe("Flow 02: Onboarding Wizard & Configuration", () => {
  test("walks through git provider, safeguards, initial pool and completes onboarding", async ({
    authedPage: page,
  }) => {
    ...
    // First render at " / " pulls the dashboard route chunk cold; under CI
    // load the router transition plus chunk fetch can outlive the default
    // 5s visibility window (seen once on the self-hosted runner).
