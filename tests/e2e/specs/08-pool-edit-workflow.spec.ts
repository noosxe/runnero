...
test.describe('Flow 08: Runner Pool Edit Workflow', () => {
  ...
  test('spares busy runners when a spawn-identity edit recycles idle ones', async ({ onboardedPage: page, request }) => {
    ...
    await expect
      .poll(async () => (await poolRunnerRows(page)).filter((r) => r.state === 'idle').length, { timeout: 30_000 })
      .toBeGreaterThanOrEqual(1);
    ...
    await expect
      .poll(async () => (await poolRunnerRows(page)).find((r) => r.name === busyRunner.name)?.state, { timeout: 60_000 })
      .toBe('busy');
    ...
    await expect
      .poll(async () => {
        ...
        return rows.find((r) => r.name === busyRunner.name)?.state === 'busy' && rows.some((r) => r.state === 'idle');
      }, { timeout: 60_000 })
      .toBe(true);
    ...
    for (const name of recycledNames) {
      await expect
        .poll(async () => (await poolRunnerRows(page)).some((r) => r.name === name), { timeout: 30_000 })
        .toBe(false);
    }

    // Cleanup: release the busy flag at the forge, wait for the runner to go
    // idle again, then drop the mirror registration
