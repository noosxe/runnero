...
func (c *Client) spawn(ctx context.Context, config orchestrator.RunnerConfig, taskType string) (string, error) {
	...
	if config.CPULimit != "" {
		nanoCPUs, err := limits.ParseCPULimit(config.CPULimit)
		if err != nil {
			return "", err
		}
	...
	if config.MemoryLimit != "" {
		memBytes, err := limits.ParseMemoryLimit(config.MemoryLimit)
		if err != nil {
			return "", err
		}
		hostConfig.Memory = memBytes
		// Memory swap cap (RUN-147): semantics mirror Docker's MemorySwap —
		// the TOTAL memory+swap allowance. "-1" = unlimited swap; otherwise
		// the parsed value (>= memory, enforced at the API layer; equal = no
		// extra swap). Unset leaves the daemon default of 2x memory.
		if config.MemorySwapLimit != "" {
			if config.MemorySwapLimit == "-1" {
				hostConfig.MemorySwap = -1
			} else {
				swapBytes, err := limits.ParseMemoryLimit(config.MemorySwapLimit)
				if err != nil {
					return "", fmt.Errorf("parsing memory swap limit: %w", err)
				}
				hostConfig.MemorySwap = swapBytes
			}
		}
	}
