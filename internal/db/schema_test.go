...
func TestInitialSchemaTablesAndSeeds(t *testing.T) {
	...
	if ver != 7 {
		t.Fatalf("database version = %d, want 7", ver)
	}
	...
	expectedTables := map[string][]string{
		...
		"runner_pools": {
			...
			"memory_swap_limit", "poll_fallback", "poll_interval_seconds",
...
func TestInitialSchemaUpDownIdempotent(t *testing.T) {
	...
	ver, err := database.Version(ctx, nil)
	if err != nil || ver != 7 {
		t.Fatalf("Version after boot = %d (err: %v), want 7", ver, err)
	}
	...
	// Migrate up again: should succeed and restore version 6
	...
	if err != nil || verUp != 7 {
		t.Fatalf("Version after Migrate up = %d (err: %v), want 7", verUp, err)
	}
