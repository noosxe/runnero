...
- **Demand Polling Fallback:** Per-pool queued-job polling fallback for webhook-driven pools (GitHub) so runners scale on workstations and NAT'd hosts without inbound webhooks — the webhook route's semantics on a bounded polling cadence. *[Design Phase]*
