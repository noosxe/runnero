...
| `SUPERVISOR_LOG_PERSISTENCE_ENABLED` | Bool | No | `true` | Mirrors the structured log stream into boot files under `<data-dir>/logs/supervisor/` and persists runner captures + removal records (`RUN-186`, `docs/28`). `false` disables all durable log artifacts; stdout behavior never changes. |
| `SUPERVISOR_LOG_SUPERVISOR_ROTATION_BYTES` | Int | No | `33554432` | Size threshold per supervisor boot file before rolling to the next (`-0001.ndjson`, …). |
| `SUPERVISOR_LOG_SUPERVISOR_MAX_FILES` | Int | No | `10` | Supervisor boot files retained (oldest removed first) by the retention sweep. |
| `SUPERVISOR_LOG_RUNNER_CAPTURE_MAX_BYTES` | Int | No | `16777216` | Tail cap on a runner's captured stdout before container removal (the end is the failure). |
| `SUPERVISOR_LOG_RUNNER_CAPTURE_TIMEOUT_SECONDS` | Int | No | `10` | Hard cap per capture attempt: forensics never delay removal. |
| `SUPERVISOR_LOG_RUNNER_MAX_FILES` | Int | No | `200` | Runner capture files retained under `<data-dir>/logs` (oldest removed first). |
| `SUPERVISOR_LOG_TOTAL_BUDGET_BYTES` | Int | No | `1073741824` | Hard budget across all durable logs; the hourly sweep enforces it oldest-first. |
...
- **Viewer Role & Multi-User Management** *[Design Phase]*: real viewer accounts with read-only observability and an admin-only user-management surface, activating the role foundation from docs/32. Design: `docs/35-viewer-role.md`.
