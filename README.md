...
- **Runner Pool Editing (M26):** Non-destructive editing of every pool configuration field — targets, quotas, runner image, labels, auth profile, and rename — through the shared create/edit wizard (pools list + pool detail Config tab). Spawn-identity edits and renames recycle the pool's *idle* runners so respawns pick up the new configuration, while busy runners finish their jobs untouched; renames are guarded on zero busy runners (`failed_precondition` otherwise) and providers are immutable. The review step shows a changed-fields diff with runner-impact banners, duplicate names surface as `already_exists`, audits record before/after restricted to changed fields, and the Renovate tab keeps working as a renovate-only shortcut (`docs/22`).
